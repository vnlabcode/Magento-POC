...
define([
    ...
    $.widget("mage.mediabrowser", {
        ...
        insert: function(event) {
            ...
            $('.rokanthemespoup').remove();
            $('.loading-old, .loading-mask').css({"z-index":"13000"});
            return $.ajax({
                ...
                $('.loading-old, .loading-mask').css({"z-index":""});
                $('.loading-mask').hide();*/
                //$('[role="dialog"]').show().addClass("rokanthemespoup").css({"z-index":"20000 !important"});
                $('[role="dialog"].clearlooks2').show().addClass("rokanthemespoup").css({"z-index":"20000 !important"});
                MediabrowserUtility.closeDialog();
                targetEl.focus();
            }, this));
        },
