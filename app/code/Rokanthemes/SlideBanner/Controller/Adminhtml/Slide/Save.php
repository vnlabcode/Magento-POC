...
class Save extends \Magento\Backend\App\Action
{
	...
	public function execute()
    {
        if ($data = $this->getRequest()->getPostValue('slide')) {
			...
			if(isset($data['slide_text']) && is_array($data['slide_text']) && !empty($data['slide_text'])){
				$save_slide = [];
				foreach ($data['slide_text'] as $key_s_t => $val_s_t) {
					$save_slide[] = $val_s_t;
				}
                $data['slide_text'] = json_encode($save_slide);
			}
