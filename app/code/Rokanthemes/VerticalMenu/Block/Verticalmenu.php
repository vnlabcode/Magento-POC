...
class Verticalmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null, $nav_fixed)
    {
        $html = '';
        $child_nav = 0;
        if(!$max_level || ($max_level && $max_level == 0) || ($max_level && $max_level > 0 && $max_level-1 >= $level)) {
            ...
            foreach($children as $child) {
                ...
                if (!$vc_menu_hide_item) {
                    ...
                    $html .= '<li class="ui-menu-item nav-'.$get_nav_class.' '.$item_class.'">';
                    if(count($sub_children) > 0) {
                    ...
                    $html .= '<a'.$a_class.' href="'.$this->_categoryHelper->getCategoryUrl($child).'">';
                    if ($level > 1 && $vc_menu_icon_img)
                    ...
                    $html .= '</span></a>';
                    if(count($sub_children) > 0) {
                        $html .= $this->getSubmenuItemsHtml($sub_children, $level+1, $max_level, $column_width, $menu_type, null, $get_nav_class);
                    }
                    $html .= '</li>';
                }
            }
    ...
    public function getVerticalMenuHtml()
    {
        ...
        foreach($categories as $category) {
            ...
            if(!$vc_menu_hide_item) {
                ...
                $custom_style = '';
                $css_fixed = array();
                if($cat_model->getData('vertcial_menu_bg_img') != ''){
                    $trim_pub = explode("media", $cat_model->getData('vertcial_menu_bg_img'));
                    $url_fixed = $cat_model->getData('vertcial_menu_bg_img');
                ...
                if($menu_type=="staticwidth" && $vc_menu_static_width){
                    $css_fixed[] = 'width: '.$vc_menu_static_width;
                }
                ...
                $item_class = 'level0 nav-'.$nav_fixed.' ';
                ...
                if ($vc_menu_icon_img)
                    $html .= '<img class="menu-thumb-icon" src="'.$vc_menu_icon_img.'" alt="'.$category->getName().'"/>';
                elseif($vc_menu_font_icon)
                    $html .= '<em class="menu-thumb-icon '.$vc_menu_font_icon.'"></em>';
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $vc_menu_cat_columns, $nav_fixed);
