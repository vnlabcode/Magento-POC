    ...
    public function getCustomBlockHtml($type='after') {
        ...
        foreach($ids as $block_id) {
            ...
            for ($offset = 0; $offset < strlen($content); $offset++) {
                if(substr($content, $offset, 4) == '<ul>') {
                    $content = substr($content, $offset + 4);
                    break;
                }
            }

            for ($offset = strlen($content); $offset > 0; $offset--) {
                if(substr($content, $offset,  5) == '</ul>') {
...
//                $content = substr($content, 0, -5);
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null, $nav_fixed = 1)
    {
        $html = '';
            ...
            foreach($children as $child) {
                ...
                if (!$rt_menu_hide_item) {
                    ...
                    if(count($sub_children) > 0)
                        $item_class .= 'parent ';
                    $html .= '<li class="ui-menu-item nav-'.$get_nav_class.' '.$item_class.'"'.$custom_style.'>';
                    if(count($sub_children) > 0) {
            ...
            if(!$rt_menu_hide_item) {
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $rt_menu_cat_columns, $nav_fixed);
